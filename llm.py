...
def _text_providers() -> list[tuple[str, str, str]]:
    """Цепочка провайдеров для текста: (base_url, api_key, model).

    Если задан UNITY2_API_KEY — сначала Gemini через Unity2,
    затем Groq (LLM_MODEL), затем быстрая запасная модель Groq.
    """
    providers: list[tuple[str, str, str]] = []
    if UNITY2_API_KEY:
        providers.append((UNITY2_BASE_URL, UNITY2_API_KEY, UNITY2_MODEL))
    if LLM_API_KEY:
        providers.append((LLM_BASE_URL, LLM_API_KEY, LLM_MODEL))
        if LLM_MODEL != _FALLBACK_MODEL:
            providers.append((LLM_BASE_URL, LLM_API_KEY, _FALLBACK_MODEL))
    return providers
...
async def llm(messages: list[dict]) -> str:
    ...
    if _has_image(messages):
        ...
        providers = [(LLM_BASE_URL, LLM_API_KEY, LLM_VISION_MODEL)]
    else:
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        for base_url, api_key, model in providers:
            for attempt in range(1, _RETRIES + 1):
                try:
                    status, data = await _call(session, base_url, api_key, model, messages)
...
1. Первый шаг — коротко и по делу
2. Второй шаг
3. Третий шаг
